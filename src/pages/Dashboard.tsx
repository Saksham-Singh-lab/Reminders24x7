    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Here's what's happening in our community.</p>
        </div>
        <div className="flex items-center gap-2 bg-white p-2 rounded-2xl shadow-sm border border-primary-100">
          <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-2 hover:bg-primary-50 rounded-xl transition-colors">
          ...
          <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-2 hover:bg-primary-50 rounded-xl transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Calendar */}
        <div className="lg:col-span-2 bg-white p-6 rounded-3xl shadow-xl shadow-primary-100/20 border border-primary-50">
          <div className="grid grid-cols-7 gap-2 mb-4">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
              <div key={day} className="text-center text-xs font-bold text-primary-300 uppercase tracking-wider py-2">
                {day}
              </div>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected ? "bg-primary-500 border-primary-500 text-white shadow-lg shadow-primary-200" : "bg-white border-transparent hover:border-primary-200 text-slate-700",
                    isTodayDate && !isSelected && "text-primary-500 font-bold"
                  )}
                >
                  ...
                  {dayReminders.length > 0 && (
                    <div className="flex gap-1">
                      {dayReminders.slice(0, 3).map((_, i) => (
                        <div key={i} className={cn("w-1.5 h-1.5 rounded-full", isSelected ? "bg-white" : "bg-primary-400")} />
                      ))}
                    </div>
                  )}
                </button>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-xl shadow-primary-100/20 border border-primary-50 flex flex-col h-full min-h-[400px]">
                ...
                {selectedDateReminders.length > 0 ? (
                  selectedDateReminders.map((reminder) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: -20 }}
                      className="p-4 bg-primary-50/50 rounded-2xl border border-primary-100 group relative"
                    >
                      <h3 className="font-bold text-slate-800 mb-1">{reminder.title}</h3>
                      <p className="text-sm text-slate-500 leading-relaxed">{reminder.description}</p>
                      {isAdmin && (
                        <button
                          onClick={() => handleDeleteReminder(reminder.id)}
                          className="absolute top-2 right-2 p-1 text-primary-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
                        >
                          <Trash2 size={16} />
                        </button>
                      )}
      ...
      <AnimatePresence>
        {showAddModal && (
          <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-slate-800">Add Reminder</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border-2 border-transparent focus:border-primary-300 focus:bg-white rounded-2xl outline-none transition-all h-32 resize-none"
                    placeholder="Don't forget to bring your admit card..."
                  />
                </div>
                <button
                  onClick={handleAddReminder}
                  className="w-full py-4 bg-primary-500 hover:bg-primary-600 text-white font-bold rounded-2xl shadow-lg shadow-primary-200 transition-all mt-4"
                >
                  Create Reminder
                </button>
              </div>
